...
public class NodeMove : MonoBehaviour
{
    ...
    public bool rotateObject, loopMovement;
    ...
    IEnumerator StartMove()
    {
        ...
        transform.localPosition = startPos + path[posID];

        while (loopMovement || posID < path.Count - 1)
        {
            //this handles the index of the path vectors, and decide the next target position
            if (((startPos + path[posID]) - transform.localPosition).sqrMagnitude < 0.01f)
            {
                if (loopMovement)
                {
            ...
            transform.localPosition = Vector3.MoveTowards(transform.localPosition, startPos + path[posID], speed * Time.deltaTime);
            ...
            if (rotateObject)
            {
                if (Time.time > nextAngleGrab)
                {
                    ...
                    Vector3 dir = (startPos + path[posID]) - transform.localPosition;
